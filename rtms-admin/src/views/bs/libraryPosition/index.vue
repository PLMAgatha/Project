...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      getonetemplate(row.id).then((res) => {
        res.data[0].positionType = String(res.data[0].positionType);
        res.data[0].positionStatus = String(res.data[0].positionStatus);
        res.data[0].isEnable = String(res.data[0].isEnable);
        res.data[0].floor = String(res.data[0].floor);
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id !== undefined) {
            updateLibraryPosition(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.dialogFormVisible = false;
              this.getList();
            });
          } else {
            // this.form.positionType = parseInt(this.form.positionType);
            // this.form.positionStatus = parseInt(this.form.positionStatus);
            // this.form.floor = parseInt(this.form.floor);
            addLibraryPosition(this.form).then((response) => {
