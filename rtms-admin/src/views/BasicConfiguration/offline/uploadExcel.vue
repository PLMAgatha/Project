...
import uploadExcel from "@/views/BasicConfiguration/offline/uploadExcel";
  ...
  data() {
    return {
      upload: {
        ...
        // 下线上传的地址
        url: process.env.VUE_APP_BASE_API + '/admin-api/bs/offlinedraw-conf/import'
      },
      fileList: [],
    }
  },
  methods: {
    ...
    importTemplate() {
      downloadPreExcel().then(response => {
        this.$download.excel(response, '下线图号模板案例.xls');
      });
    },
    // 文件上传中处理
