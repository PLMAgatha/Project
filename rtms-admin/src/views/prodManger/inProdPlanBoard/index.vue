...
<script>
import { getplanlistBoard } from "@/api/prodManger/inProdPlan";
export default{
  data(){
return{
  nowDate: '',//实时时间
  ...
  methods: {
    ...
    getList() {
      //未执行
      getplanlistBoard(this.queryParams).then(
        response => {
          this.templateList = response.data;
        }
      );
      //执行中
      getplanlistBoard(this.queryParams1).then(
        response => {
          this.templateList1 = response.data;
        }
      );
      //异常
      getplanlistBoard(this.queryParams2).then(
        response => {
