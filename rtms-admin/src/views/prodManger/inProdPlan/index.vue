<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="inprodplanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="空箱是否流转" align="center" prop="emptyBoxHoming" :show-overflow-tooltip="true"
                       :formatter="emptyBoxHomingformatter"/>
    ...
    <!--推迟-->
    <postpone ref="postpone" :title="postponedata.title" :linedata="postponedata.linedata"
              @refresh="refresh"></postpone>
  </div>
</template>

<script>
import editstatus from "@/views/prodManger/inProdPlan/editstatus.vue";
import tochannel from "@/views/prodManger/inProdPlan/tochannel.vue";
import eferredrecord from "../inProdPlan/eferredrecord.vue";
import interposition from "@/views/prodManger/inProdPlan/interposition.vue";
import postpone from "@/views/prodManger/inProdPlan/postpone.vue";
import inadvance from "@/views/prodManger/inProdPlan/inadvance.vue";
import {getplanlist, delall, getExcel, updown, recalculate} from "@/api/prodManger/inProdPlan";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getplanlist(this.queryParams).then(
        response => {
    ...
    handleDelete() {
      if (this.arr.length == 0) {
        ...
        if (alldel) {
          ...
          this.$message.warning('请对待配送的数据进行删除')
        }
      }
    },
    //更改任务状态
    ...
    handleExcel() {
      if (this.arr.length == 0) {
        ...
        this.$modal.confirm('是否确认导出选中备料生产计划?').then(() => {
          this.exportLoading = true;
          return getExcel(ids);
        }).then(response => {
          this.$download.excel(response, '备料生产计划.xls');
